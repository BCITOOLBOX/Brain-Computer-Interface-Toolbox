...
classdef gen_tr <handle & SuperClass
    ...
    properties(Access = public)
        ...
        commonmode       %subtraction mode modifier
...
if isempty(this.ft) || isempty(this.ftmrk)
    [ft,ftmrk]=ftprep(this.datafiles,this.predt,this.postdt,this.chid,this.commonmode,this.ftid);
end

%make features
[trialExamples,trialLabels]=make_features(this.ft,this.ftmrk,this.ftid);


            
        end
        
        function PrepareTrials(this)
...
if isfield(this.ft,'tridx')
    trialExamples=trialExamples(this.ft.tridx,:);
    trialLabels=trialLabels(this.ft.tridx);
end

%constrain examples to contain only the desired 'targets'
this.targets=sort(this.targets);
idx=ismember(trialLabels,this.targets);
...
fprintf('#########################\n');
        ...
        function PrepareTrainValidationTtestDatasets(this)
...
if nargin<9 || isempty(this.testid)
...
if xvalsequential
    ...
    trainid=((1:nn)/nn)<this.xvalThr;
else
    fprintf('Random x-validation split\n');
    trainid=rand(1,nn)<this.xvalThr;
end

%training dataset, randomize order
idx=find(trainid & ~this.testid);
...
testTargets=trialLabels(idx);
        ...
        function PrepareClassifierObject(this)
            %perform feature ranking and selection
[ftidx,ranks,nid]=parse_ftid(this.ftid,this.ft,trialExamples,trialLabels,this.targets);
...
if nid>0
    ...
    clobj=this.funcTrain(ctexamples,cttargets,cvexamples,cvtargets);
else
    ...
    for nid=dnn:dnn:length(ranks)
        ...
        xtest=this.funcClassify(clobj,cvexamples);
        ...
        xtest=this.funcClassify(clobj,ctexamples);
